...
@section('content')
   ...
   <section class="content-header">
    ...
    <section class="content">
        <form action="{{ route('admin.gallery.update') }}" role="form" method="post" enctype="multipart/form-data">
            @csrf
                    ...
                    <div class="box box-primary">
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                <label>Tiêu đề</label>
                                <textarea name="description" class="form-control textarea" rows="8" placeholder="Mô tả">{{ $image->image_desc }}</textarea>
                            </div>
                            <div class="form-group @error('file') has-error @enderror">
                                <label>Ảnh thu nhỏ</label>
                                <div class="preview-image" style="width: 250px; height: 168px;">
                                    <img src="@if(!empty($image->image_picture)) {{ Storage::disk('public')->url($image->image_picture) }} @else {{ Storage::disk('public')->url('uploads/default.png') }} @endif" alt="Avatar" id="preview">
                                </div>
                            ...
                            <div class="form-group">
                                <label>Thứ tự</label>
                                <input type="number" class="form-control" name="image_priority" value="{{ $image->image_priority }}"/>
                            </div>
                            <div class="form-group">
                                <label>Trạng thái</label>
                                <select class="form-control" name="image_status">
                                    <option value="1" @if($image->image_status) selected @endif>Hiển thị</option>
                                    <option value="0" @if(!$image->image_status) selected @endif>Không hiển thị</option>
                                </select>
                            </div>
                        </div>
                    </div>
