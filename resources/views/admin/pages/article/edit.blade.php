...
@section('content')
   <!-- Content Header (Page header) -->
    ...
    <section class="content">
        <form action="{{ route('admin.article.postEdit') }}" role="form" method="post" enctype="multipart/form-data">
            @csrf
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="general">
                                <div class="form-group @error('title') has-error @enderror">
                                    ...
                                    <input type="text" class="form-control" placeholder="Nhập tiêu đề bài viết" value="{{ $article->title }}" name="title">
                                    @error('title')
                                    ...
                                    <select name="categories[]" class="form-control select2" multiple="multiple" data-placeholder=" Chọn danh mục" style="width: 100%;">
                                        @foreach($categories as $item)
                                            <option
                                                value="{{ $item->id }}"
                                                @php
                                                    echo in_array($item->id, $categorySelected) ? 'selected' : '';
                                                @endphp
                                            >{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="summary" rows="8" class="form-control textarea" placeholder="Nhập mô tả">{{ $article->summary }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label>Nội dung bài viết</label>
                                    <textarea name="content" rows="8" class="form-control" id="editor1" placeholder="Nhập nội dung bài viết">{{ $article->content }}</textarea>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" placeholder="Vị trí" value="{{ $article->sort_order }}" name="sortOrder">
                                </div>
                                <div class="form-group">
                                    <label>Trạng thái</label>
                                    <select name="status" class="form-control">
                                        <option value="1" @if($article->status == 1) selected @endif>Công khai</option>
                                        <option value="0" @if($article->status == 0) selected @endif>Bản nháp</option>
                                    </select>
                                </div>
                            </div>
                            <!-- /.tab-pane -->
                            <div class="tab-pane" id="seo">
                                <div class="form-group @error('metaTitle') has-error @enderror">
                                    ...
                                    <input type="text" class="form-control" placeholder="Nhập tiêu đề" value="{{ $article->meta_title }}" name="metaTitle">
                                    @error('metaTitle')
                                        <span class="help-block">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" rows="5" cols="40" name="metaDescription" placeholder="Nhập mô tả">{{ $article->meta_description }}</textarea>
                                </div>
                                <div class="form-groupr">
                                    <label>Từ khóa</label>
                                    <textarea class="form-control" rows="5" cols="30" name="metaTagKeywords" placeholder="Nhập từ khóa tìm kiếm trên Google">{{ $article->meta_keyword }}</textarea>
                                </div>
                            </div>
                            <!-- /.tab-pane -->
                        </div>
